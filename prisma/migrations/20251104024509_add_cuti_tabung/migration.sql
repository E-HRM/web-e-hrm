...
-- AlterTable
ALTER TABLE `agenda_kerja` ADD COLUMN `created_by_snapshot` VARCHAR(255) NULL,
    MODIFY `status` ENUM('teragenda', 'diproses', 'ditunda', 'selesai') NOT NULL;

-- AlterTable
ALTER TABLE `cuti_konfigurasi` ADD COLUMN `cuti_tabung` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `id_user` CHAR(36) NOT NULL;

-- AlterTable
ALTER TABLE `kunjungan` ADD COLUMN `created_by_snapshot` VARCHAR(255) NULL;

-- AlterTable
ALTER TABLE `user` ADD COLUMN `catatan_delete` LONGTEXT NULL,
    ADD COLUMN `status_cuti` ENUM('aktif', 'nonaktif') NOT NULL DEFAULT 'aktif';
