...
export async function sendPengajuanIzinSakitEmailNotifications(req, pengajuan) {
  ...
  const deeplink = `/pengajuan-izin-sakit/${pengajuan.id_pengajuan_izin_sakit}`;
  ...
  const handoverClean = cleanHandoverFormat(pengajuan.handover);
  ...
  if (approverEmails.length) {
    const subject = `[E-HRM] Permintaan Persetujuan Izin Sakit (${pemohonName})`;
    const html = makeEmailHtml({
      ...
      fields,
      buttonUrl: url,
      buttonText: 'Buka Pengajuan Izin Sakit',
    });
    ops.push(sendBatch({ from, to: approverEmails, subject, html }));
  }

  if (handoverEmails.length) {
    const subject = `[E-HRM] Anda ditunjuk sebagai handover izin sakit (${pemohonName})`;
    const html = makeEmailHtml({
  ...
  if (pemohonEmail) {
    const subject = `[E-HRM] Pengajuan Izin Sakit Terkirim (${tanggalLabel})`;
    const html = makeEmailHtml({
      ...
      fields,
      buttonUrl: url,
      buttonText: 'Lihat Pengajuan Izin Sakit',
    });
