...
function validateAutoGeneratedScheduleOnCreate({ nominal_pinjaman, sisa_saldo, status_pinjaman, tanggal_selesai, generatedTanggalSelesai }) {
  if (decimalToScaledBigInt(sisa_saldo, DECIMAL_SCALE) !== decimalToScaledBigInt(nominal_pinjaman, DECIMAL_SCALE)) {
    throw new Error("Pinjaman baru harus memiliki 'sisa_saldo' sama dengan 'nominal_pinjaman' agar jadwal cicilan otomatis dapat dibuat.");
  }

  if (status_pinjaman !== 'AKTIF') {
    throw new Error("Pinjaman baru yang dibuat dari endpoint ini harus berstatus 'AKTIF'.");
  }

  if (tanggal_selesai && toDateOnlyComparable(tanggal_selesai) !== toDateOnlyComparable(generatedTanggalSelesai)) {
    throw new Error("Field 'tanggal_selesai' harus sama dengan jatuh tempo cicilan terakhir yang dihasilkan otomatis.");
  }
}
...
export async function POST(req) {
  ...
  try {
    ...
    const sisa_saldo = normalizeDecimalString(body?.sisa_saldo === undefined ? nominal_pinjaman : body?.sisa_saldo, 'sisa_saldo', DECIMAL_SCALE, { min: '0' });

    const tanggal_mulai = parseRequiredDateOnly(body?.tanggal_mulai, 'tanggal_mulai');
    const tanggal_selesai = parseDateOnly(body?.tanggal_selesai, 'tanggal_selesai');

    const status_pinjaman = body?.status_pinjaman === undefined ? (decimalToScaledBigInt(sisa_saldo, DECIMAL_SCALE) === 0n ? 'LUNAS' : 'AKTIF') : normalizeEnum(body?.status_pinjaman, STATUS_PINJAMAN_VALUES, 'status_pinjaman');
    ...
    validateAutoGeneratedScheduleOnCreate({
      nominal_pinjaman,
      sisa_saldo,
      status_pinjaman,
      tanggal_selesai,
      generatedTanggalSelesai: generatedSchedule.tanggal_selesai,
    });
    ...
    const created = await db.$transaction(async (tx) => {
      const createdPinjaman = await tx.pinjamanKaryawan.create({
        data: {
          ...
          sisa_saldo,
          tanggal_mulai,
          tanggal_selesai: generatedSchedule.tanggal_selesai,
          status_pinjaman,
          catatan,
        },
        select: {
    ...
    return NextResponse.json({ message: 'Pinjaman karyawan berhasil dibuat beserta jadwal cicilannya.', data: created }, { status: 201 });
  } catch (err) {
    if (err instanceof Error) {
      if (
        ...
        err.message.includes('jadwal cicilan otomatis') ||
        err.message.includes('jatuh tempo cicilan terakhir')
      ) {
        return NextResponse.json({ message: err.message }, { status: 400 });
      }
    }
