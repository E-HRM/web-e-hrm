...
export const STATUS_PAYOUT_VALUES = new Set([
  "DRAFT",
  "DISETUJUI",
  "DIPOSTING_KE_PAYROLL",
  "DITAHAN",
]);
...
export function sumDecimalStrings(a, b, scale = MONEY_SCALE) {
  const total =
    decimalToScaledBigInt(a, scale) + decimalToScaledBigInt(b, scale);
...
export function enrichPayout(data) {
  ...
  const detailRows = Array.isArray(data.payout_detail)
    ? data.payout_detail
    : [];
  const totalShareDetail = detailRows.reduce(
    (acc, item) =>
      acc +
      decimalToScaledBigInt(String(item.nominal_share || "0"), MONEY_SCALE),
    0n,
  );
  const totalOssDetail = detailRows.reduce(
    (acc, item) =>
      acc + decimalToScaledBigInt(String(item.nominal_oss || "0"), MONEY_SCALE),
    0n,
  );
  const totalShareDitahan = detailRows.reduce(
    (acc, item) =>
      item?.ditahan
        ? acc +
          decimalToScaledBigInt(String(item.nominal_share || "0"), MONEY_SCALE)
  ...
  const periodeTerkunci =
    data.periode_konsultan?.status_periode ===
    STATUS_PERIODE_KONSULTAN_TERKUNCI;
  const posted = data.status_payout === "DIPOSTING_KE_PAYROLL";
  const payrollImmutable = Boolean(
    data.periode_payroll?.status_periode &&
    STATUS_PERIODE_PAYROLL_IMMUTABLE.has(data.periode_payroll.status_periode),
  );
  ...
  return {
    ...
    detail_summary: Array.isArray(data.payout_detail)
      ? {
          ...
          total_share_detail: scaledBigIntToDecimalString(
            totalShareDetail,
            MONEY_SCALE,
          ),
          total_share_ditahan: scaledBigIntToDecimalString(
            totalShareDitahan,
            MONEY_SCALE,
          ),
          total_oss_detail: scaledBigIntToDecimalString(
            totalOssDetail,
            MONEY_SCALE,
          ),
        }
    ...
    business_state: {
      ...
      bisa_diubah:
        ...
        !Boolean(data.periode_konsultan?.deleted_at) &&
        !periodeTerkunci &&
        !posted,
      bisa_dihapus:
        !Boolean(data.periode_konsultan?.deleted_at) &&
...
export function ensurePayoutMutable(existing) {
  if (
    existing.status_payout === "DIPOSTING_KE_PAYROLL" ||
    existing.diposting_pada
  ) {
    throw createHttpError(
      409,
      "Payout konsultan sudah diposting ke payroll dan tidak boleh diubah.",
    );
  }
  ...
  if (existing.periode_konsultan?.deleted_at) {
    throw createHttpError(
      409,
      "Periode konsultan untuk payout ini sudah dihapus.",
    );
  }
  ...
  if (
    ...
    throw createHttpError(
      409,
      "Periode konsultan untuk payout ini sudah terkunci.",
    );
  }
}
...
export function ensurePayoutDeletable(existing) {
  if (
    ...
    throw createHttpError(
      409,
      "Payout konsultan sudah diposting ke payroll dan tidak boleh dihapus.",
    );
  }
...
export async function findConflictingPayout(
  tx,
  { id_periode_konsultan, id_user, excludeId = null },
) {
...
function sumTransactionFields(transactions, fieldName) {
  return transactions.reduce(
    (acc, item) =>
      acc + decimalToScaledBigInt(String(item[fieldName] || "0"), MONEY_SCALE),
    0n,
  );
}
    ...
    payout_detail: {
      some: {
        ...
        payout: {
          is: {
            deleted_at: null,
            status_payout: "DIPOSTING_KE_PAYROLL",
          },
        },
      },
    },
...
async function collectEligibleTransactions(
  tx,
  {
    id_periode_konsultan,
    id_user,
    periode_konsultan = null,
  ...
  if (!transactions.length) {
    throw new Error(
      "Belum ada transaksi konsultan aktif atau carry-forward yang siap dipayout untuk user dan periode tersebut.",
    );
  }

  const transactionIds = transactions.map(
    (item) => item.id_transaksi_konsultan,
  );
  ...
  const blockingDuplicateLinks = duplicateLinks.filter(
    (item) =>
      !(
        item.ditahan &&
        item.payout?.status_payout === "DIPOSTING_KE_PAYROLL"
      ),
  );

  if (blockingDuplicateLinks.length) {
    const duplicatedIds = blockingDuplicateLinks
      .map((item) => item.id_transaksi_konsultan)
      .join(", ");
    throw new Error(
      `Sebagian transaksi sudah terhubung ke payout lain: ${duplicatedIds}.`,
    );
  }
...
export async function resolvePayoutPayload(
  tx,
  input,
  existing = null,
  actor = null,
) {
  const id_periode_konsultan = hasOwn(input, "id_periode_konsultan")
    ? input.id_periode_konsultan
    : existing?.id_periode_konsultan;
  const id_user = hasOwn(input, "id_user") ? input.id_user : existing?.id_user;
  const id_periode_payroll = hasOwn(input, "id_periode_payroll")
    ? input.id_periode_payroll
    : (existing?.id_periode_payroll ?? null);
  ...
  const nominal_penyesuaian = hasOwn(input, "nominal_penyesuaian")
    ? input.nominal_penyesuaian
    : existing?.nominal_penyesuaian !== undefined &&
        existing?.nominal_penyesuaian !== null
      ? String(existing.nominal_penyesuaian)
      : "0.00";

  const status_payout = hasOwn(input, "status_payout")
    ? input.status_payout
    : (existing?.status_payout ?? "DRAFT");

  const catatan = hasOwn(input, "catatan")
    ? input.catatan
  ...
  const transactionIdSet = new Set(
    transactions.map((item) => item.id_transaksi_konsultan),
  );

  const invalidWithheldTransactionId = withheldTransactionIds.find(
    (id) => !transactionIdSet.has(id),
  );

  if (invalidWithheldTransactionId) {
    throw new Error(
      `Transaksi ditahan '${invalidWithheldTransactionId}' tidak tersedia pada kumpulan transaksi payout yang aktif.`,
    );
  }
  ...
  const totalShareBigInt = sumTransactionFields(
    enrichedTransactions,
    "nominal_share",
  );
  const totalOssBigInt = sumTransactionFields(
    enrichedTransactions,
    "nominal_oss",
  );
  const totalDitahanBigInt = enrichedTransactions.reduce(
    (acc, item) =>
      item.ditahan
        ? acc +
          decimalToScaledBigInt(String(item.nominal_share || "0"), MONEY_SCALE)
  ...
  const total_share = scaledBigIntToDecimalString(
    totalShareBigInt,
    MONEY_SCALE,
  );
  const nominal_ditahan = scaledBigIntToDecimalString(
    totalDitahanBigInt,
    MONEY_SCALE,
  );
  const nominal_dibayarkan = sumDecimalStrings(
    subtractDecimalStrings(total_share, nominal_ditahan, MONEY_SCALE),
    nominal_penyesuaian,
    MONEY_SCALE,
  );
  ...
  if (decimalToScaledBigInt(nominal_dibayarkan, MONEY_SCALE) < 0n) {
    throw new Error(
      "Perhitungan 'nominal_dibayarkan' tidak boleh menghasilkan nilai negatif.",
    );
  }

  if (status_payout === "DIPOSTING_KE_PAYROLL" && !id_periode_payroll) {
    throw new Error(
      "Field 'id_periode_payroll' wajib diisi saat status_payout = 'DIPOSTING_KE_PAYROLL'.",
    );
  }
  ...
  return {
    ...
    summary: {
      ...
      detail_ditahan_count: enrichedTransactions.filter((item) => item.ditahan)
        .length,
    },
  };
}

export async function replacePayoutDetails(
  tx,
  id_payout_konsultan,
  transactions,
) {
...
export async function setTransaksiPostingStatusByPayout(
  tx,
  id_payout_konsultan,
  posted,
) {
...
async function ensurePayrollPostingTarget(tx, { id_periode_payroll, id_user }) {
  ...
  if (!payroll) {
    throw new Error(
      "Payroll karyawan untuk user dan periode payroll tersebut tidak ditemukan.",
    );
  }
  ...
  if (
    payroll.status_payroll &&
    STATUS_PAYROLL_IMMUTABLE.has(payroll.status_payroll)
  ) {
    throw new Error(
  ...
  if (
    ...
    STATUS_PERIODE_PAYROLL_IMMUTABLE.has(payroll.periode.status_periode)
  ) {
...
async function getDefinitionForPayoutPosting(
  tx,
  ...
  if (!id_definisi_komponen_payroll) {
    throw createHttpError(
      400,
      "Field 'id_definisi_komponen_payroll' wajib diisi saat posting payout ke payroll.",
    );
  }
...
function buildPayoutPayrollItemNote(payout) {
  const nomorInduk = payout.user?.nomor_induk_karyawan
    ? ` (${payout.user.nomor_induk_karyawan})`
    : "";
  return `Posting payout konsultan untuk ${payout.user?.nama_pengguna || "user"}${nomorInduk}.`;
}

function buildPayoutPayrollItemKey(id_payout_konsultan) {
...
export async function recalculatePayrollTotals(tx, id_payroll_karyawan) {
  ...
  if (pendapatanBersih < 0n) {
    throw new Error(
      "Perubahan posting payout membuat pendapatan_bersih payroll bernilai negatif.",
    );
  }
...
export async function upsertPayrollItemForPayout(
  tx,
  payout,
  actor = null,
  options = {},
  ...
  const definisi = await getDefinitionForPayoutPosting(
    tx,
    options?.id_definisi_komponen_payroll,
  );
  ...
  const payload = {
    ...
    id_definisi_komponen_payroll:
      definisi?.id_definisi_komponen_payroll ?? null,
    id_user_pembuat: actor?.id ?? null,
    kunci_idempoten: buildPayoutPayrollItemKey(payout.id_payout_konsultan),
    tipe_komponen: String(definisi?.tipe_komponen?.nama_tipe_komponen || "").trim(),
    arah_komponen: String(definisi?.arah_komponen || "").trim().toUpperCase(),
    ...
    urutan_tampil:
      options?.urutan_tampil ?? existingItem?.urutan_tampil ?? 900,
  ...
  const totals = await recalculatePayrollTotals(
    tx,
    payrollTarget.id_payroll_karyawan,
  );
