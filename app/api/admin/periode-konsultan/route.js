...
export async function GET(req) {
  ...
  try {
    ...
    const [total, data] = await Promise.all([
      db.periodeKonsultan.count({ where }),
      db.periodeKonsultan.findMany({
        where,
    ...
    console.error('GET /api/admin/periode-konsultan error:', err);
...
export async function POST(req) {
  ...
  try {
    ...
    const existing = await db.periodeKonsultan.findFirst({
      where: { tahun, bulan },
      select: {
        id_periode_konsultan: true,
        deleted_at: true,
    ...
    if (existing && existing.deleted_at !== null) {
      const restored = await db.periodeKonsultan.update({
        where: { id_periode_konsultan: existing.id_periode_konsultan },
        data: payload,
        select: buildSelect(),
      });
      ...
      return NextResponse.json(
        {
          message: 'Periode konsultan berhasil dipulihkan dan diperbarui.',
          data: restored,
        },
        { status: 200 },
      );
    ...
    const created = await db.periodeKonsultan.create({
      data: payload,
      select: buildSelect(),
    });

    return NextResponse.json({ message: 'Periode konsultan berhasil dibuat.', data: created }, { status: 201 });
  } catch (err) {
    if (err?.code === 'P2002') {
      return NextResponse.json({ message: 'Periode konsultan untuk tahun dan bulan tersebut sudah ada.' }, { status: 409 });
    }
    ...
    console.error('POST /api/admin/periode-konsultan error:', err);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
