...
/* =========================
   MEDIA PREVIEW (force preview)
========================= */
function getUrlFromFileLike(f) {
  if (!f) return null;
  if (typeof f === "string") return f;

  // prefer url-ish fields; jangan pakai name kalau cuma label
...
function getLabelFromFileLike(file, url) {
  if (file && typeof file === "object") {
    return file.label || file.filename || (file.name && !String(file.name).startsWith("http") ? file.name : null) || null;
  }
    ...
    <div className="mt-2">
      {shouldTryImage && !imgFailed ? (
        ...
        <div className="mt-2">
          ...
          <a href={url} target="_blank" rel="noreferrer">
            <AppTypography.Text className="text-slate-700 underline">
              {label}
            </AppTypography.Text>
          </a>
        </div>
...
export default function PocketMoneyDetailModal({
  open,
  onClose,
  data,
  onApprove,
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="rounded-2xl ring-1 ring-slate-100 bg-white p-4">
                ...
                {/* ✅ langsung preview image */}
                <MediaPreview file={data.paymentProof || data.paymentProofUrl} forceImagePreview />
              </div>
