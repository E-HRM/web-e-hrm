...
function toPublicUrl(input) {
  ...
  const base = (typeof ApiEndpoints?.FileBaseURL === 'string' && ApiEndpoints.FileBaseURL) || (typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_FILE_BASE_URL) || '';
...
function getApprovalsArray(item) {
  const candidates = [item?.approvals, item?.approval_izin_tukar_hari, item?.ApprovalIzinTukarHari, item?.approvalPengajuanTukarHari];
...
function guessPolaOptionsUrl() {
  try {
    if (typeof ApiEndpoints?.GetPolaKerjaOptions === 'function') return ApiEndpoints.GetPolaKerjaOptions();
    if (ApiEndpoints?.GetPolaKerjaOptions) return ApiEndpoints.GetPolaKerjaOptions;

    if (typeof ApiEndpoints?.GetPolaKerja === 'function') return ApiEndpoints.GetPolaKerja({ page: 1, perPage: 999 });
    if (ApiEndpoints?.GetPolaKerja) return ApiEndpoints.GetPolaKerja;

    if (typeof ApiEndpoints?.GetPolaOptions === 'function') return ApiEndpoints.GetPolaOptions();
...
function normalizeAttachments(item) {
  ...
  const pushUrl = (maybeUrl, nameFallback = 'Lampiran') => {
    ...
    const fileName = raw.split('/').pop()?.split('?')[0] || nameFallback;
    result.push({ url, name: fileName });
  };

  const arrayCandidates = [item?.attachments, item?.lampiran, item?.berkas, item?.files, item?.file_kelengkapan, item?.kelengkapan, item?.kelengkapan_files, item?.dokumen_kelengkapan, item?.dokumen, item?.documents];

  for (const arr of arrayCandidates) {
    if (Array.isArray(arr)) {
      for (const it of arr) {
        const rawUrl = (typeof it === 'string' && it) || it?.url || it?.link || it?.path || it?.file_url || it?.file || it?.lokasi_file || it?.filepath || '';
        const name = it?.name || it?.filename || it?.original_name || it?.originalName || it?.nama_file || it?.file_name || undefined;
        pushUrl(rawUrl, name);
      }
    }
  }

  const singleCandidates = [item?.file_kelengkapan, item?.file_kelengkapan_url, item?.lampiran_tukar_hari_url, item?.lampiran_url, item?.bukti_url, item?.file_url, item?.kelengkapan_url, item?.dokumen_url];
...
function mapItemToRow(item) {
  ...
  const hariIzin = pairs[0]?.izin || item?.hari_izin || item?.tanggal_izin || item?.tgl_izin || item?.tanggal || null;

  const hariPengganti = pairs[0]?.pengganti || item?.hari_pengganti || item?.tanggal_pengganti || item?.tgl_pengganti || null;
  ...
  const rawHandover = item?.handover ?? item?.handover_text ?? item?.handover_desc ?? item?.handover_description ?? item?.keterangan_handover ?? '—';
  ...
  const buktiUrl = item?.lampiran_izin_tukar_hari_url ?? item?.lampiran_izin_tukar_hari ?? null;
  ...
  return {
    ...
    buktiUrl: buktiUrl ? toPublicUrl(buktiUrl) : null,
    attachments,
...
export default function useTukarHariViewModel() {
  ...
  const countKey = useCallback(
    (status) =>
      ApiEndpoints.GetPengajuanTukarHariMobile({
        status,
        page: 1,
        perPage: 1,
        all: 1,
  ...
  const filteredData = useMemo(() => {
    ...
    return rows.filter((d) => {
      ...
      return [d.nama, d.jabatanDivisi, d.kategori, d.keperluan, d.hariIzin, d.hariPengganti, d.handover, handoverNames, attNames].join(' ').toLowerCase().includes(term);
    });
  }, [rows, search]);
  ...
  async function ensureApprovalId(row) {
    ...
    try {
      const detailUrl = typeof ApiEndpoints.GetPengajuanTukarHariDetail === 'function' ? ApiEndpoints.GetPengajuanTukarHariDetail(row.id) : ApiEndpoints.GetPengajuanTukarHariMobile({ id: row.id, all: 1 });
  ...
  const approve = useCallback(
    async (id, note = null, idPolaKerjaPengganti = null) => {
      ...
      if (!approvalId) {
        AppMessage.error('Tidak menemukan id approval. Pastikan API mengembalikan approvals / my_pending_approval_id atau aktifkan endpoint detail.');
        return;
      }
      ...
      try {
        ...
        AppMessage.success('Pengajuan tukar hari disetujui');
  ...
  const reject = useCallback(
    async (id, note) => {
      ...
      if (!approvalId) {
        AppMessage.error('Tidak menemukan id approval pada pengajuan ini.');
        return false;
      }
      ...
      try {
        ...
        AppMessage.success('Pengajuan tukar hari ditolak');
        await Promise.all([mutate(), swrCntPending.mutate(), swrCntApproved.mutate(), swrCntRejected.mutate()]);
        return true;
      } catch (e) {
        AppMessage.error(e?.message || 'Gagal menyimpan keputusan.');
