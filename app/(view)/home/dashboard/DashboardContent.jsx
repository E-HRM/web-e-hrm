...
function MiniCalendar({
  ...
  onPrevMonth,
  onNextMonth,
  onSelectDay, // callback ketika tanggal diklik
}) {
        ...
        {cells.map((day, idx) => {
          ...
          const handleClick = () => {
            ...
            if (typeof onSelectDay === "function") {
              const dateObj = new Date(year, monthIndex, day); // lokal, hanya untuk tampilan
...
export default function DashboardContent() {
  ...
  const perfDivisionOptions =
    vm.perfDivisionOptions || [{ label: "--Semua Divisi--", value: "" }];
  const currentPerfRows = vm.currentPerfRows || [];

  // state untuk modal detail cuti per tanggal
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="grid grid-cols-12 gap-4">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Bar dataKey="value" radius={[10, 10, 10, 10]}>
                      {vm.miniBars.map((_, i) => (
                        <Cell key={i} fill={miniColors[i % 5]} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <div className="col-span-12 grid grid-cols-12 gap-4">
          {[
            ...
            {
              value: vm.statCards.presensi,
              label: "Presensi",
              bg: "bg-slate-50",
              ring: "ring-slate-200/60", // <-- sudah dibenerin, satu baris
              text: "text-slate-600",
              icon: (
                <svg viewBox="0 0 24 24" className="h-6 w-6" fill="currentColor">
            ...
            <div key={idx} className="col-span-6 md:col-span-3">
              <div className="h-full rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 p-4 flex items-center gap-3 hover:shadow-md transition">
                <div
                  className={`h-10 w-10 rounded-xl ring-1 flex items-center justify-center ${s.bg} ${s.ring} ${s.text}`}
                >
                  {s.icon}
                </div>
            ...
            {vm.leaveList.length === 0 ? (
              ...
              vm.leaveList.map((u, idx) => (
                ...
                >
                  ...
                  <div className="col-span-3 text-right text-[11px] text-gray-500">
                    —
                  </div>
                </div>
              ))
      ...
      {/* Modal detail cuti per tanggal */}
      <Modal
        open={!!selectedDateInfo}
        onCancel={() => setSelectedDateInfo(null)}
        footer={null}
        title={
          selectedDateInfo
            ? `Karyawan cuti • ${selectedDateInfo.date.toLocaleDateString(
                "id-ID",
                {
                  weekday: "long",
                  day: "numeric",
      ...
      >
        {selectedDateInfo && selectedDateInfo.items.length > 0 ? (
          ...
          <div className="text-xs text-gray-500">
            Tidak ada karyawan cuti di tanggal ini.
          </div>
        )}
      </Modal>
    </div>
