...
/** Tampilkan waktu persis seperti DB (tidak digeser zona). */
export const showFromDB = (v, fmt = "DD MMM YYYY HH:mm") => {
  if (!v) return "-";
  const s = String(v);
  // ISO dgn Z/offset → render via UTC supaya angkanya tidak berubah
...
export default function useKunjunganKalenderViewModel() {
  ...
  // Guard agar tidak setState bila range-nya sama (mencegah loop)
  const setRange = useCallback((r) => {
    ...
    setViewRange((prev) => {
      ...
      if (prevStart === nextStart && prevEnd === nextEnd) return prev; // tidak berubah → jangan update
      return { start: r.start, end: r.end };
    });
  }, []);

  // ---- list kunjungan by range (date-only)
  const listUrl = useMemo(() => {
    if (!viewRange.start || !viewRange.end) return null;
    const p = new URLSearchParams();
    p.set("startDate",  asDateOnly(viewRange.start) + " 00:00:00");
    p.set("endDate",    asDateOnly(viewRange.end)   + " 23:59:59");
    p.set("pageSize", "1000");
    p.set("orderBy", "tanggal");
    p.set("sort", "desc");
    // Pastikan di ApiEndpoints ada GetKunjungan (GET /api/admin/kunjungan-klien)
  ...
  // ---- buat rencana kunjungan untuk BANYAK user (multi-select)
  const createPlansForUsers = useCallback(async ({ userIds = [], tanggal, jam_mulai, jam_selesai, deskripsi }) => {
    ...
    for (const uid of userIds) {
      const payload = {
        id_user: uid,
        ...(DEFAULT_KATEGORI_ID ? { id_kategori_kunjungan: DEFAULT_KATEGORI_ID } : {}),
      ...
      // Pastikan di ApiEndpoints ada CreateKunjungan (POST /api/admin/kunjungan-klien)
