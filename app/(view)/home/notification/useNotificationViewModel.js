...
export default function useNotificationViewModel() {
  const swrKey = ApiEndpoints.GetNotificationsRecent({
    days: 7,
    types: ['pengajuan_cuti', 'izin_tukar_hari', 'pengajuan_izin_sakit', 'pengajuan_izin_jam'].join(','),
  });
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR(swrKey, fetchWithToken, {
    refreshInterval: 30000,
  });

  const items = useMemo(() => (Array.isArray(apiResponse?.data) ? apiResponse.data : []), [apiResponse]);
  ...
  const markAllRead = useCallback(async () => {
    try {
      ...
      AppMessage.error('Gagal menandai semua notifikasi');
    }
  }, [mutate]);
  ...
  const markOneRead = useCallback(
    async (id) => {
      ...
      try {
        const token = Cookies.get('token');
        const res = await fetch(ApiEndpoints.MarkNotificationRead(idStr), {
          method: 'PUT',
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        });
        ...
        AppMessage.error('Gagal menandai notifikasi');
      }
    },
    [mutate]
  );
