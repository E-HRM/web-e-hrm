...
import { ProfileOutlined, ClockCircleOutlined, EditOutlined, DeleteOutlined, PlusOutlined, EllipsisOutlined } from '@ant-design/icons';
...
export default function AgendaCalendarContent() {
  ...
  const [createProgress, setCreateProgress] = useState({ running: false, sent: 0, total: 0 });
  ...
  const statusColor = useCallback((st) => (st === 'selesai' ? 'fc-chip fc-chip--done' : st === 'ditunda' ? 'fc-chip fc-chip--hold' : st === 'teragenda' ? 'fc-chip fc-chip--plan' : 'fc-chip fc-chip--proc'), []);
  ...
  const openEdit = useCallback(
    (fcEvent) => {
      ...
      if (fcEvent.end && end.diff(start, 'day') === 1 && start.hour() === 0 && start.minute() === 0 && end.hour() === 0 && end.minute() === 0) {
  ...
  const confirmBulkDeleteSimilar = useCallback(async () => {
    ...
    try {
      ...
      if (!n) {
        AppMessage.info('Tidak ada agenda serupa ditemukan.');
        return;
      }
  ...
  const renderEventContent = useCallback(
    (info) => {
      ...
      const projectName = info.event.extendedProps?.agenda?.nama_agenda || info.event.extendedProps?.agenda_name || info.event.title || '-';

      const uid = info.event.extendedProps?.id_user;
      const userFromMap = uid ? vm.getUserById(uid) : null;
      const userName = info.event.extendedProps?.user?.nama_pengguna || userFromMap?.nama_pengguna || info.event.extendedProps?.user?.name || info.event.extendedProps?.user?.email || '';
  ...
  const urgencyChip = useMemo(() => {
    ...
    const raw = detailEvent.extendedProps?.raw || {};
    const rawVal = raw.kebutuhan_agenda ?? raw.kebutuhan ?? raw.urgensi ?? raw.prioritas ?? raw.agenda?.kebutuhan_agenda ?? raw.agenda_kerja?.kebutuhan_agenda ?? null;
  ...
  const formFields = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        controlProps: { showTime: { format: 'HH:mm', defaultValue: defaultStartTime }, format: 'YYYY-MM-DD HH:mm', className: 'w-full' },
      },
      {
        name: 'end',
        label: 'Selesai',
        type: 'datetime',
        controlProps: { showTime: { format: 'HH:mm', defaultValue: defaultEndTime }, format: 'YYYY-MM-DD HH:mm', className: 'w-full' },
      },
    ];
  }, [defaultEndTime, defaultStartTime, editId, handleClearUsers, handleSelectAllUsers, totalUsers, usersLabelNode, usersSelected.length, vm.agendaOptions, vm.userOptions]);
  ...
  return (
    <div className='p-4'>
      <div className='relative rounded-2xl border border-slate-200 bg-white shadow-xl p-3'>
        ...
        {vm.reloadingEvents && !vm.loadingInitialEvents && <div className='absolute right-3 top-3 z-10 rounded-full bg-white/90 px-3 py-1 text-[11px] text-slate-500 shadow-sm'>Memuat ulang agenda…</div>}
        ...
        <FullCalendar
          ...
          select={(sel) => openCreate(sel.startStr)}
        ...
        {!detailEvent ? null : (
          <>
            <div className='flex items-start gap-3'>
              ...
              <div className='min-w-0'>
                ...
                <div className='mt-3 flex flex-wrap items-center gap-2'>
                  {detailEvent.extendedProps?.status ? <span className={statusColor(detailEvent.extendedProps.status)}>{detailEvent.extendedProps.status}</span> : null}
                  {urgencyChip?.label ? <span className={`fc-chip fc-chip--urg-${urgencyChip.level}`}>{urgencyChip.label}</span> : null}
                  {detailEvent.extendedProps?.agenda?.nama_agenda ? <span className='fc-chip fc-chip--clip1'>{detailEvent.extendedProps.agenda.nama_agenda}</span> : null}
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: isQuillEmpty(descHtml) ? '<span style="color:#94a3b8">—</span>' : descHtml }}
            />
        ...
        footer={
          <div className='flex items-center justify-between gap-3'>
            ...
            <div className='flex gap-2'>
              ...
              >
                {createProgress.running && !editId ? `Buat (${createProgress.sent}/${createProgress.total})` : createProgress.running && editId ? 'Menyimpan...' : editId ? 'Simpan' : 'Buat'}
              </AppButton>
            </div>
          </div>
        }
        ...
        <AppForm
          ...
          onFinish={async (values) => {
            ...
            try {
              ...
              if (newId) form.setFieldsValue({ id_agenda: newId });
      ...
      >
        {typeof confirmCfg.content === 'string' ? <div className='text-sm text-slate-700'>{confirmCfg.content}</div> : confirmCfg.content}
      </AppModal>
