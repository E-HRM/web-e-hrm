...
import FormComponent from '@/app/(view)/component_shared/AppForm';

import { fetcher } from '@/app/utils/fetcher';
import { ApiEndpoints } from '@/constrainst/endpoints';

/* ========= Helpers ========= */
const safeName = (name) => (name || 'semua-karyawan').trim().replace(/[\/\\?%*:|"<>]/g, '-');
const fmtDateForFilename = (d) => dayjs(d).format('YYYY/MM/DD').replaceAll('/', '／');
...
function flatten(rec) {
  ...
  const istirahat_mulai_raw = a.jam_istirahat_mulai ?? a.mulai_istirahat ?? a.break_start ?? a.istirahat_in ?? null;
  const istirahat_selesai_raw = a.jam_istirahat_selesai ?? a.selesai_istirahat ?? a.break_end ?? a.istirahat_out ?? null;
  ...
  const jabatanRaw = user?.jabatan ?? user?.posisi ?? user?.position ?? user?.role_jabatan ?? user?.nama_jabatan ?? null;

  const departement = typeof departemenRaw === 'string' ? { nama_departement: departemenRaw } : departemenRaw || null;
  const jabatan = typeof jabatanRaw === 'string' ? { nama_jabatan: jabatanRaw } : jabatanRaw || null;

  const fotoUser = normalizePhotoUrl(user?.foto_profil_user ?? user?.foto ?? user?.avatarUrl ?? user?.avatar ?? user?.photoUrl ?? user?.photo ?? null);
...
export default function ExportExcelModal({ open, onClose, rowsAll = [], employeeOptions: employeeOptionsProp, lokasiOptions: lokasiOptionsProp }) {
  ...
  const fetchAllUsers = useCallback(async () => {
    ...
    while (true) {
      const url = typeof ApiEndpoints.GetUsers === 'function' ? ApiEndpoints.GetUsers({ page, perPage, orderBy: 'nama_pengguna', sort: 'asc' }) : `${ApiEndpoints.GetUsers}?page=${page}&perPage=${perPage}&orderBy=nama_pengguna&sort=asc`;
  ...
  const { data: usersAll, isLoading: usersLoading } = useSWR(shouldFetch ? 'export:users:allpages' : null, fetchAllUsers, { revalidateOnFocus: false });
  ...
  const { data: locRes, isLoading: locsLoading } = useSWR(shouldFetch ? (typeof ApiEndpoints.GetLocation === 'function' ? ApiEndpoints.GetLocation({ perPage: 1000 }) : `${ApiEndpoints.GetLocation}?perPage=1000`) : null, fetcher);
  ...
  const fallbackFromRows = useMemo(() => {
    ...
    rowsAll.forEach((r) => {
      ...
      const addLoc = (loc) => {
        if (!loc) return;
        const label = loc.name || (Number.isFinite(loc.lat) && Number.isFinite(loc.lon) ? `Lat ${loc.lat.toFixed(5)}, Lon ${loc.lon.toFixed(5)}` : '');
  ...
  const { employeeOptions, lokasiOptions } = useMemo(() => {
    const finalEmp = (employeeOptionsProp && employeeOptionsProp.length && employeeOptionsProp) || (apiEmployeeOptions.length && apiEmployeeOptions) || fallbackFromRows.employeeOptions;

    const finalLoc = (lokasiOptionsProp && lokasiOptionsProp.length && lokasiOptionsProp) || (apiLokasiOptions.length && apiLokasiOptions) || fallbackFromRows.lokasiOptions;

    return { employeeOptions: finalEmp, lokasiOptions: finalLoc };
  }, [employeeOptionsProp, lokasiOptionsProp, apiEmployeeOptions, apiLokasiOptions, fallbackFromRows]);
    ...
    const dateField =
      tab === 'single'
        ? {
            ...
            controlProps: { format: 'DD/MM/YYYY', className: 'w-full' },
          }
        : {
            name: 'range',
            label: 'Tanggal',
    ...
    return [
      dateField,
      {
        ...
        controlProps: {
          ...
          filterOption: (input, opt) => (opt?.label ?? '').toLowerCase().includes(String(input).toLowerCase()),
          virtual: true,
          listHeight: 360,
  ...
  }, [tab, lokasiOptions, employeeOptions, locsLoading, usersLoading]);
  ...
  const onFinish = useCallback(
    async (v) => {
      try {
          ...
          while (true) {
            const url =
              typeof ApiEndpoints.GetAbsensiRecords === 'function'
                ...
                : `${ApiEndpoints.GetAbsensiRecords}?from=${start.format('YYYY-MM-DD')}&to=${end.format('YYYY-MM-DD')}&perPage=${pageSize}&page=${page}`;

            const api = await fetcher(url);
            const batch = Array.isArray(api?.data) ? api.data : Array.isArray(api?.items) ? api.items : [];
            pool.push(...batch);

            const totalPages = api?.pagination?.totalPages ?? api?.meta?.totalPages ?? null;
            if (totalPages) {
              if (page >= totalPages) break;
              page += 1;
            } else {
        ...
        if (!filtered.length) {
          AppMessage.warning('Tidak ada data pada rentang & filter yang dipilih.');
          setSubmitting(false);
          return;
        }
        ...
        if ((users || []).length === 1) {
          employeeName = employeeLabelById.get(String(users[0])) || 'karyawan';
        } else if ((users || []).length > 1) {
          employeeName = `multi-${users.length}-karyawan`;
        } else {
          const uniq = new Set(filtered.map((x) => x?.user?.nama_pengguna || x?.user?.name || x?.user?.email).filter(Boolean));
          if (uniq.size === 1) employeeName = Array.from(uniq)[0];
        }

        const sameDay = start.isSame(end, 'day');
        const datePart = sameDay ? fmtDateForFilename(start) : `${fmtDateForFilename(start)} - ${fmtDateForFilename(end)}`;
        ...
        AppMessage.success('File Excel berhasil dibuat.');
        setSubmitting(false);
        onClose?.();
      } catch (e) {
        console.error(e);
        setSubmitting(false);
        AppMessage.error('Gagal mengekspor data.');
      }
    },
    [tab, rowsAll, onClose, employeeLabelById]
  );
